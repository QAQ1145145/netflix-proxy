...
sudo iptables -t nat -A PREROUTING -i ${IFACE} -p tcp --dport 80 -j REDIRECT --to-port 8080\
  ...
  && sudo iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport 11000 -j ACCEPT\
  && sudo iptables -A INPUT -p udp -m state --state NEW -m udp --dport 11000 -j ACCEPT\
  && sudo iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport 12222 -j ACCEPT\
  && sudo iptables -A INPUT -p udp -m state --state NEW -m udp --dport 12222 -j ACCEPT\
  && sudo iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport 30000 -j ACCEPT\
  && sudo iptables -A INPUT -p udp -m state --state NEW -m udp --dport 30000 -j ACCEPT\
  && sudo iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport 42350 -j ACCEPT\
